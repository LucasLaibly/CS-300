...
/**
 * Heart of the Jungle Park program, setups the board and objects created/removed
 * appropriately
 * 
 * @author Mouna Kacem
 * @author Lucas Laibly
...
public class JunglePark extends PApplet
{
  ...
  public void setup() {
    ...
    //we want to AddNewButton (anb) objects for both Tiger and Deer onto the board
    //as well as the clear board button
    AddAnimalButton anbTiger = new AddAnimalButton("Tiger", 43, 16, this);
    AddAnimalButton anbDeer = new AddAnimalButton("Deer", 129, 16, this);
    ClearButton clear = new ClearButton(215,16, this);

    //once the objects are created, place them into the list of ALL objects
    listGUI.add(anbTiger);
    listGUI.add(anbDeer);
    listGUI.add(clear);
  ...
  public void clear()
  {
    //for the entire baord
    for (int i = 0; i < listGUI.size(); i++)
    {
      //if there is an animal object
      if (listGUI.get(i) instanceof Animal)
      {
        //we remove it and reduce i
        listGUI.remove(i);
        i--;
      }
    }
